    ...
    struct npc_1v1arenaAI : public ScriptedAI
    {
        ...
        bool OnGossipHello(Player* player)
        {
            ...
            if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|TInterface\\icons\\Ability_Mage_TormentOfTheWeak:30|tLeave queue 1v1 Arena|r", GOSSIP_SENDER_MAIN, 3, "|TInterface\\icons\\INV_Misc_QuestionMark:30|tAre you sure?|r", 0, false);
            else
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|TInterface\\icons\\Achievement_BG_winWSG:30|tSign up 1v1 Arena (unrated)|r", GOSSIP_SENDER_MAIN, 20);

            if (player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5)) == 0)
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|TInterface\\icons\\INV_Misc_GroupNeedMore:30|tCreate new 1v1 Arenateam|r", GOSSIP_SENDER_MAIN, 1, "|TInterface\\icons\\INV_Misc_QuestionMark:30|tCreate 1v1 arenateam?|r", config_Costs, false);
            else
            {
                if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5) == false)
                {
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|TInterface\\icons\\INV_Misc_GroupLooking:30|tSign up 1v1 Arena (rated)|r", GOSSIP_SENDER_MAIN, 2);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|TInterface\\icons\\Spell_Frost_Stun:30|tDisband arenateam|r", GOSSIP_SENDER_MAIN, 5, "|TInterface\\icons\\INV_Misc_QuestionMark:30|tAre you sure?|r", 0, false);
                }

                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|TInterface\\icons\\Spell_Holy_PrayerofSpirit:30|tShow statistics|r", GOSSIP_SENDER_MAIN, 4);
            }
        ...
        bool OnGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId)
        {
            ...
            case 4: // get statistics
            {
                ...
                if (at)
                {
                    ...
                    s << "\n|TInterface\\icons\\INV_Jewelcrafting_AzureHare:10:10|tRating:|r " << at->GetStats().Rating;
                    s << "\n|TInterface\\icons\\INV_Jewelcrafting_AzureSerpent:10:10|tRank:|r " << at->GetStats().Rank;
                    s << "\n|TInterface\\icons\\INV_Jewelcrafting_AzureBoar:10:10|tSeason Games:|r " << at->GetStats().SeasonGames;
                    s << "\n|TInterface\\icons\\INV_Jewelcrafting_GoldenOwl:10:10|tSeason Wins:|r " << at->GetStats().SeasonWins;
                    s << "\n|TInterface\\icons\\INV_Jewelcrafting_CrimsonOwl:10:10|tWeek Games:|r " << at->GetStats().WeekGames;
                    s << "\n|TInterface\\icons\\INV_Jewelcrafting_DragonsEye02:10:10|tWeek Wins:|r " << at->GetStats().WeekWins;
